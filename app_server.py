...
function renderTable() {
  ...
  let rows = extractedItems.map(item => {
    ...
    <tr${rowClass}>
      ...
      <td class="sale-price">${escapeHtml(item.salePrice)}</td>
      ${matchCell}
  ...
  resultsEl.innerHTML = `
    <table class="results-table">
      <thead><tr><th>File</th><th>Store</th><th>Product Name</th><th>Price</th><th>Sale Price</th>${matchHeader}<th>Note</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>${unmatchedHtml}`;
...
function copyAllCSV() {
  ...
  const header = ['File', 'Store', 'Product Name', 'Price', 'Sale Price', ...(hasMatch ? ['Match'] : []), 'Note'].join(',');
  const rows = extractedItems.map(it => {
    const cols = [it.file, it.store, it.name, it.price, it.salePrice];
